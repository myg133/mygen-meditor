import { aD as Gt, s as Wt, g as Kt, q as Ht, p as Xt, a as Yt, b as Zt, _ as S, D as St, I as Jt, d as rt, as as Qt, V as $t, W as te, X as ee, e as ne, z as se, F as ie, G as re } from "./index-DWbaqf43.js";
...
function ye(t, n) {
  ...
  for (const y of t) {
    ...
    y.size + Dt >= Math.min(e[m].size, e[x].size) && (d = 0), i[m].push({ set: x, size: y.size, weight: d }), i[x].push({ set: m, size: y.size, weight: d });
  }
  const r = [];
...
function be(t = {}) {
  ...
  function l(p) {
    ...
    const R = U.selectAll(".venn-area").data(h, (I) => I.sets), O = R.enter().append("g").attr(
      "class",
    ...
    let T = p;
    z && typeof T.transition == "function" ? (T = D(p), T.selectAll("path").attrTween("d", A)) : T.selectAll("path").attr("d", (I) => wt(I.sets.map((L) => k[L])), x);
    ...
    const C = D(R.exit()).remove();
    typeof R.transition == "function" && C.selectAll("path").attrTween("d", A);
    const Y = C.selectAll("text").attr("x", s / 2).attr("y", e / 2);
    return y !== null && (V.style("font-size", "0px"), F.style("font-size", y), Y.style("font-size", "0px")), { circles: k, textCentres: N, nodes: R, enter: O, update: T, exit: C };
  }
  return l.wrap = function(p) {
    ...
    return arguments.length ? (v = p === "default" ? et : p === "logRatio" ? Rt : p, l) : v;
  }, l;
}
function ot(t, n) {
  return function(s) {
...
function Me(t, n = {}) {
  ...
  let x = e(t, {
    lossFunction: s === "default" || !s ? et : s === "logRatio" ? Rt : s,
    distinct: d
  });
  i && (x = Ct(x, r, o));
...
var gt = function() {
  var t = /* @__PURE__ */ S(function(w, g, b, v) {
    ...
    parse: /* @__PURE__ */ S(function(g) {
      var b = this, v = [0], l = [], p = [null], h = [], j = this.table, k = "", N = 0, X = 0, B = 2, U = 1, q = h.slice.call(arguments, 1), z = Object.create(this.lexer), A = { yy: {} };
      for (var R in this.yy)
        Object.prototype.hasOwnProperty.call(this.yy, R) && (A.yy[R] = this.yy[R]);
      ...
      S(D, "lex");
      for (var T, F, C, Y, I = {}, L, G, Z, H; ; ) {
        if (F = v[v.length - 1], this.defaultActions[F] ? C = this.defaultActions[F] : ((T === null || typeof T > "u") && (T = D()), C = j[F] && j[F][T]), typeof C > "u" || !C.length || !C[0]) {
          var J = "";
          H = [];
          for (L in j[F])
...
Expecting ` + H.join(", ") + ", got '" + (this.terminals_[T] || T) + "'" : J = "Parse error on line " + (N + 1) + ": Unexpected " + (T == U ? "end of input" : "'" + (this.terminals_[T] || T) + "'"), this.parseError(J, {
        ...
        switch (C[0]) {
          case 1:
            v.push(T), p.push(z.yytext), h.push(z.yylloc), v.push(C[1]), T = null, X = z.yyleng, k = z.yytext, N = z.yylineno, O = z.yylloc;
            break;
          case 2:
            if (G = this.productions_[C[1]][1], I.$ = p[p.length - G], I._$ = {
              ...
              return Y;
            G && (v = v.slice(0, -1 * G * 2), p = p.slice(0, -1 * G), h = h.slice(0, -1 * G)), v.push(this.productions_[C[1]][0]), p.push(I.$), h.push(I._$), Z = j[v[v.length - 2]][v[v.length - 1]], v.push(Z);
            break;
          case 3:
            return !0;
        }
...
}, "addStyleData"), Ae = /* @__PURE__ */ S(() => mt, "getStyleData"), it = /* @__PURE__ */ S((t) => t.map((n) => nt(n)), "normalizeIdentifierList"), De = /* @__PURE__ */ S((t) => {
  const s = it(t).filter((e) => !bt.has(e));
  if (s.length > 0)
    throw new Error(`unknown set identifier: ${s.join(", ")}`);
}, "validateUnionIdentifiers"), Re = /* @__PURE__ */ S(() => pt, "getTextData"), Ce = /* @__PURE__ */ S(() => vt, "getCurrentSets"), Ne = /* @__PURE__ */ S(() => It, "getIndentMode"), Oe = /* @__PURE__ */ S((t) => {
  It = t;
...
var Be = /* @__PURE__ */ S((t, n, s, e) => {
  ...
  for (const A of j) {
    const R = $([...A.data.sets].sort());
    k.set(R, A);
  }
  m.length > 0 && Ut(r, k, l, m, w, x);
  const N = $t(o.background || "#f4f4f4");
  l.selectAll(".venn-circle").each(function(A, R) {
    var L;
    const O = rt(this), V = $([...A.sets].sort()), D = x.get(V), T = (D == null ? void 0 : D.fill) || a[R % a.length] || o.primaryColor;
    O.classed(`venn-set-${R % 8}`, !0);
    const F = (D == null ? void 0 : D["fill-opacity"]) ?? 0.1, C = (D == null ? void 0 : D.stroke) || T, Y = (D == null ? void 0 : D["stroke-width"]) || `${5 * w}`;
    if (u && h) {
      const G = k.get(V);
      if (G && G.circles.length > 0) {
        const Z = G.circles[0], H = h.circle(Z.x, Z.y, Z.radius * 2, {
          ...
          stroke: C,
          strokeWidth: parseFloat(String(Y))
        });
        O.select("path").remove(), (L = O.node()) == null || L.insertBefore(H, O.select("text").node());
      }
    } else
      O.select("path").style("fill", T).style("fill-opacity", F).style("stroke", C).style("stroke-width", Y).style("stroke-opacity", 0.95);
    const I = (D == null ? void 0 : D.color) || (N ? te(T, 30) : ee(T, 30));
    O.select("text").style("font-size", `${48 * w}px`).style("fill", I);
  }), u && h ? l.selectAll(".venn-intersection").each(function(A) {
    var T;
    const R = rt(this), P = $([...A.sets].sort()), V = x.get(P), D = V == null ? void 0 : V.fill;
    if (D) {
      const F = R.select("path"), C = F.attr("d");
      if (C) {
        const Y = h.path(C, {
          roughness: 0.7,
          seed: f,
        ...
        (T = I == null ? void 0 : I.parentNode) == null || T.insertBefore(Y, I), F.remove();
      }
    } else
      R.select("path").style("fill-opacity", 0);
    R.select("text").style("font-size", `${48 * w}px`).style("fill", (V == null ? void 0 : V.color) ?? o.vennSetTextColor ?? b);
  ...
  const X = v.append("g").attr("transform", `translate(0, ${g})`), B = l.select("svg").node();
  if (B && "childNodes" in B)
    for (const A of [...B.childNodes])
      (z = X.node()) == null || z.appendChild(A);
...
function Ut(t, n, s, e, i, r) {
  ...
  for (const [y, d] of u.entries()) {
    ...
    const b = Math.max(80 * i, w * 2 * 0.95), v = Math.max(60 * i, w * 2 * 0.95), h = (m.data.label && m.data.label.length > 0 ? Math.min(32 * i, w * 0.25) : 0) + (d.length <= 2 ? 30 * i : 0), j = x - b / 2, k = M - v / 2 + h, N = Math.max(1, Math.ceil(Math.sqrt(d.length))), X = Math.max(1, Math.ceil(d.length / N)), B = b / N, U = v / X;
    for (const [q, z] of d.entries()) {
      const A = q % N, R = Math.floor(q / N), O = j + B * (A + 0.5), P = k + U * (R + 0.5);
      o && g.append("rect").attr("class", "venn-text-debug-cell").attr("x", j + B * A).attr("y", k + U * R).attr("width", B).attr("height", U).attr("fill", "none").attr("stroke", "teal").attr("stroke-width", 1 * i).attr("stroke-dasharray", `${4 * i} ${3 * i}`);
      const V = B * 0.9, D = U * 0.9, T = g.append("foreignObject").attr("class", "venn-text-node-fo").attr("width", V).attr("height", D).attr("x", O - V / 2).attr("y", P - D / 2).attr("overflow", "visible"), F = (a = r.get(z.id)) == null ? void 0 : a.color, C = T.append("xhtml:span").attr("class", "venn-text-node").style("display", "flex").style("width", "100%").style("height", "100%").style("white-space", "normal").style("align-items", "center").style("justify-content", "center").style("text-align", "center").style("overflow-wrap", "normal").style("word-break", "normal").text(z.label ?? z.id);
      F && C.style("color", F);
    }
  }
}
